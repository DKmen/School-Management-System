...
module.exports = {
    ...
    getAllExams: async (req, res) => {
        try {
            const exams = await Exam.find({}).populate(['teacher', 'subject']);
    ...
    getRelevenceExams: async (req, res) => {
        try {
            ...
            const exams = await Exam.find({ class: classid }).populate(['teacher', 'subject']);
            return res.status(200).json({
                error: false,
                data: exams
            });
    ...
    getExamsByClass: async (req, res) => {
        try {
            const { id } = req.params;
            const exams = await Exam.find({ class: { _id: id } }).populate(['teacher', 'subject']);
            return res.status(200).json({
                error: false,
                data: exams
            });
